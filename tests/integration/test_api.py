...
def test_worker_supports_outline_generation_mode(base_url: str):
    docs = get_documents(base_url)
    response = requests.post(
        f"{base_url}/api/v1/courses/generate-from-documents",
        json={
            "title": f"Outline course {unique_suffix()}",
            "documentIds": [docs[0]["id"]],
    ...
    assert generated_course.status_code == 200, generated_course.text
    generated_body = generated_course.json()
    assert "outline" in generated_body["tags"]
    assert "Сгенерированная структура курса" in generated_body["description"]
